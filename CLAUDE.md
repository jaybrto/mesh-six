...
- Research sub-workflow uses `whenAny()` from `@microsoft/durabletask-js` (standalone import, NOT `ctx.whenAny()`) for replay-safe timer/event racing. Never use `Promise.race` with Dapr workflow tasks — it breaks replay determinism.
